...
- **Observation:** The model performs well on DoS (F1=0.87) and Normal
  ...
  on U2R (F1=0.11, recall=0.06). This confirms the class imbalance problem that
...
| **5** | **Feature engineering** | **XGBoost** | **SMOTE in ImbPipeline** | **0.9453** | **0.6331** |
...
**Analysis:** The gap is large but expected. It is not the
result of overfitting or test-set leakage. The gap arises from a fundamental property of the
NSL-KDD benchmar as KDDTest+  R2L and U2R attack types
...
3. **SMOTETomek with conservative sampling targets** hurt vs. plain SMOTE
